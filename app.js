...
const text = [
  ...
  /* Select three elements the value input, form and the article where the result is going to be shown */
  ...
  form.addEventListener("submit",function(e){
    ...
    // if the input is less than or equal to 0 return 1 paragraph
    // if the input is greater than 9 then return 1 paragraph
    // If the value is an empty string (nothing input) return 1 paragraph.

    if(isNaN(value)||value<=0||value>9){
        let random = Math.floor(Math.random()*text.length);
        result.innerHTML = ` <p class="text-center mt-2 text-green-500">${text[random]}</p>`;
    }
    else {
        let paragraphs = text.slice(0,value);
        // console.log(paragraphs);
       paragraphs = paragraphs.map(function(item){
          return  ` <p class="text-center mt-2 text-green-500">${item}</p>`;
        }).join("");
        console.log(paragraphs);
